...
void PinMechanism::setState(PinMechanismState nextState) {
    if (state != nextState) {
        logger.subsystemState("Pin", toString(nextState));
    }
    state = nextState;
}

PinMechanismState PinMechanism::getState() const {
    return state;
}

void PinMechanism::debug() const {
    logger.subsystemState("Pin", toString(state));
}
